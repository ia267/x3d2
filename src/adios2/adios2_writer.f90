module m_adios2_writer
    ...
    contains
        ...
        procedure, private :: write_scalar_real    !! Writes scalar double precision real data
        procedure, private :: write_array_2d_real  !! Writes 2d array double precision real data
        procedure, private :: writer_array3d_real  !! Writes 3d array double precision real data
...
contains
    ...
    subroutine writer_array3d_real(self, name, data, shape_dims, start_dims, count_dims)
        ...
        character(len=*), intent(in) :: name
        real(kind=real64), dimension(:,:,:), intent(in) :: data
        integer(kind=int64), dimension(ndims_3d), intent(in) :: shape_dims, start_dims, count_dims
        type(adios2_variable) :: var

        print*, "writing ADIOS2 3d array"
        call adios2_define_variable(var, self%io, name, adios2_type_dp, ndims_3d, &
                                    shape_dims, start_dims, count_dims, &
                                    adios2_constant_dims, self%ierr)
    ...
    subroutine write_fields(self, timestep, fields, names, shape_dims, start_dims, count_dims)
        !! write multiple fields in one go
        class(adios2_writer_t), intent(inout) :: self
